...
router.post("/user/signIn", async (req, res) => {
  ...
  if (!customToken) {
    return res.status(400).json({ errorMessage: "데이터가 올바르지 않습니다." });
  }
...
const postFirebaseFunction = user => axios.post("https://us-central1-todaykeyword.cloudfunctions.net/createCustomToken", user).catch(error => console.log(error));
